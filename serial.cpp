...
void Serial::Input_discard(void)
{
	if (!port_is_open) return;
#if defined(__GNUC__) || defined(MACOSX)
	// does this really work properly (and is it thread safe) on __GNUC__??
	tcflush(port_fd, TCIFLUSH);
#elif defined(WINDOWS)
...
int Serial::Set_control(int dtr, int rts)
{
	...
	// on __GNUC__, where is this actually documented?
	int bits;
...
#if defined(__GNUC__)
// All __GNUC__ serial port device names.  Hopefully all of them anyway.  This
...
wxArrayString Serial::port_list()
{
	wxArrayString list;
#if defined(__GNUC__)
	// This is ugly guessing, but __GNUC__ doesn't seem to provide anything else.
	// If there really is an API to discover serial devices on __GNUC__, please
	// email paul@pjrc.com with the info.  Please?
	// The really BAD aspect is all ports get DTR raised briefly, because __GNUC__
	...
	while ((f = readdir(dir)) != NULL) {
		...
		close(fd);
		// unfortunately, __GNUC__ always raises DTR when open is called.
